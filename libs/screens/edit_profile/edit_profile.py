...
class EditProfile(MDScreen):
    ...
    def check_connection(self, *args):
        # Faz uma requisição a um site confiável (Google) para verificar a conexão
        UrlRequest(
            "https://www.google.com",
            on_success=self.connection_success,
            on_error=self.connection_failed,
            on_failure=self.connection_failed,
            timeout=5
        )

    def connection_success(self, request, result):
