...
class PrincipalScreen(MDScreen):
    ...
    def caminho_imagem(self):
        url = f'https://api-imagem.onrender.com/get-imagem?name=1'
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            return data['message']
    ...
    def salvar_imagem(self):
        ...
        url = f'https://api-imagem.onrender.com/upload-image?name={self.user()}&id=1'
        ...
        if response.status_code == 200:
            ...
            self.ids.img_perfil.source = result['secure_url']

    def seguidores(self):
        url = 'https://api-perfil.onrender.com/get-seguidores?id=1'
        response = requests.get(url)

        try:
            data = response.json()
            print(data)
            return data['informações']

        except:
            return '0'

    def seguindo(self):
        url = 'https://api-perfil.onrender.com/get-seguindo?id=1'
        response = requests.get(url)
    ...
    def nome(self):
        url = 'https://api-perfil.onrender.com/get-name?id=1'
        response = requests.get(url)

        try:
            data = response.json()
            print(data)
            return f'{str(data['informações'])[:15]}..'
    ...
    def text_bio(self):
        url = 'https://api-perfil.onrender.com/get-bio?id=1'
        response = requests.get(url)
        ...
        try:
            ...
            return str(data['informações']).replace('/n', '\n')
    ...
    def alterar_bio(self, instance):
        server = threading.Thread(target=self.url_bio)
        server.start()
    ...
    def adicionando_perfis(self):
        ...
        # Primeiro, adicione seu próprio perfil na frente, se ele existir na lista
        for item in data:
            if item['nome'] == self.user():  # Verifica se o nome é o seu
                # Cria o MDCard para o seu próprio perfil
                card = MDCard(
                    MDRelativeLayout(
                        AsyncImage(
                            source=item['perfil'],
                ...
                # Adiciona o card no topo (primeiro)
                self.ids.perfis.add_widget(card, index=0)
                break  # Sai do loop depois de adicionar seu perfil

        # Agora, adiciona os outros perfis que não são o seu
        for item in data:
            if item['nome'] != self.user():
                ...
                # Adiciona os outros perfis
                self.ids.perfis.add_widget(card)
