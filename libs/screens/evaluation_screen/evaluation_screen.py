...
class EvaluationScreen(MDScreen):
    ...
    def next_punctuality(self):
        ...
        punctuality = screen_manager.get_screen('AssesPunctuality')
        punctuality.employee_name = self.employee_name
        punctuality.employee_function = self.employee_function
        punctuality.avatar = self.avatar
        punctuality.contractor = self.salary
        punctuality.method_salary = self.method_salary
        punctuality.assess = self.assess
        punctuality.coexistence = self.coexistence
        punctuality.punctuality = self.punctuality
        punctuality.efficiency = self.efficiency
        punctuality.scale = self.scale
        punctuality.days_work = self.days_work

        screen_manager.current = 'AssesPunctuality'

    def next_efficiency(self):
        app = MDApp.get_running_app()
        screen_manager = app.root
        screen_manager.transition = SlideTransition(direction='left')
        punctuality = screen_manager.get_screen('AssesEfficiency')
        punctuality.employee_name = self.employee_name
        punctuality.employee_function = self.employee_function
        punctuality.avatar = self.avatar
        punctuality.contractor = self.salary
        ...
        screen_manager.current = 'AssesEfficiency'

    def next_coexistence(self):
        app = MDApp.get_running_app()
        screen_manager = app.root
        screen_manager.transition = SlideTransition(direction='left')
        punctuality = screen_manager.get_screen('AssesCoexistence')
        punctuality.employee_name = self.employee_name
        punctuality.employee_function = self.employee_function
        punctuality.avatar = self.avatar
        punctuality.contractor = self.salary
        ...
        screen_manager.current = 'AssesCoexistence'
