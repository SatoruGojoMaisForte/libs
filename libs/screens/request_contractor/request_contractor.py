...
class RequestContractor(MDScreen):
            ...
            if self.key in received['key']:
                ...
                for request in requests:
                    ...
                    UrlRequest(
                        ...
                        on_success=lambda req, result, employee_key=request, r=key, result_function=received: self.employee(req, result, employee_key, r, result_function)
                    )
    ...
    def create_profile_card(self, name, profession, rating, av, email, city, skills, state, sumary, telephone, key_employee, key_request, resulti):
        ...
        profile_button.on_release = lambda name_employee=name, perfil=av, city_employee=city, function=profession, email_employee=email, skill=skills, state_employee=state, summary=sumary, telefone=telephone, key_employees=key_employee, keys=key_request, result=resulti: self._upload_employee(name_employee, perfil, city_employee, function, email_employee, skills, state_employee, summary, telefone, key_employees, keys, result)
        button_layout.add_widget(profile_button)

        return card_layout

    def _upload_employee(self, name, avatar, city, profession, email, skills, state, sumary, telefone, key_employee, key_request, result):
