...
class LoginScreen(MDScreen):
    ...
    def verificar_nome(self):
        ...
        try:
            UrlRequest(
                ...
                timeout=5,  # Timeout padrão de 5 segundos
                method='GET'
            )
    ...
    def error_request_name(self, req, error):
        self.contador_tentativas += 1  # Incrementa o contador de tentativas
        print("Erro na requisição:", error)

        if self.contador_tentativas < 3:  # Limite de tentativas
            Clock.schedule_once(self.verificar_nome, 5)  # Tenta novamente após 5 segundos
        else:
            self.exibir_erro_usuario()  # Exibe mensagem de erro após tentativas esgotadas
