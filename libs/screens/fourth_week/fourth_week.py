...
class FourthWeek(MDScreen):
    ...
    def upload_week(self, start_date=None):
        ...
        # Lista para armazenar todos os dias da semana
        week_days = []

        # Itera sobre os 7 dias da semana
        for i in range(7):
            current_day = week_start + timedelta(days=i, weeks=3)
            week_days.append(current_day.date())  # Adiciona a data no formato date
    ...
    def get_week_dates(self, start_date=None):
        ...
        # Semana seguinte
        next_week_start = week_start + timedelta(weeks=3)
        next_week_end = week_end + timedelta(weeks=3)
    ...
    def database_step_two(self, instance, employees):
        for key, employe in employees.items():
            if employe['Name'] == self.employee_name and employe['contractor'] == self.contractor:
                self.database_step_three(key)

    def database_step_three(self, key):
        ...
        UrlRequest(
            ...
            on_success=self.database_step_four
        )

    def database_step_four(self, req, result):
