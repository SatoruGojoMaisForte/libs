...
class InitScreen(MDScreen):
    ...
    def email_exists(self, req, result):
        ...
        MDSnackbar(
            ...
            theme_bg_color='Custom',
            background_color=get_color_from_hex('#0267C1')
        ).open()
    ...
    def next_perfil_employee(self, result):
        ...
        try:
            if not result:
                self.show_error("Não foi possível carregar os dados tente como contratante")
                return
    ...
    def next_perfil(self, result):
        ...
        try:
            if not result:
                self.show_error("Não foi possível carregar os dados tente como funcioanario")
                return
