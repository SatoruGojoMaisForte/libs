        ...
        try:
            for key, item in result.items():
                try:
                    ...
                    if not isinstance(requests, list):  # Garante que requests seja uma lista
                        requests = []
                except (ValueError, SyntaxError) as e:
                    print(f"Erro ao processar requests em {key}: {e}")
                    requests = []  # Define um valor padrão e continua
