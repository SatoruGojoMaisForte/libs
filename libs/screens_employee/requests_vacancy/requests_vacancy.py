...
class RequestsVacancy(MDScreen):
    ...
    def workflow(self, screen):
        ...
        self.upload_requests()  # Isso já vai limpar os widgets e adicionar os novos
    ...
    def add_requests(self, req, requests):
        self.ids.main_scroll.clear_widgets()  # Limpa os widgets uma vez só aqui
        added_any = False
        ...
        # Só adiciona o label se nenhum card foi adicionado
        if not added_any:
            label = MDLabel(
                text='\nNenhuma solicitação',
                pos_hint={'center_x': 0.5, 'center_y': 0.5},
    ...
    def vacancy(self):
        ...
        vac.ids.vacancy.active = True
        vac.ids.perfil.active = False
        if hasattr(vac.ids, 'notification') and hasattr(vac.ids.notification, 'active'):
            vac.ids.notification.active = False
    ...
    def perfil(self):
        ...
        # Define o estado de navegação global
        perfil.current_nav_state = 'perfil'
        if hasattr(perfil.ids, 'notification') and hasattr(perfil.ids.notification, 'active'):
            perfil.ids.notification.active = False
